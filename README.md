...
Run a tagged slice (exactly one tag per test — see [`.cursor/rules/playwright-conventions.mdc`](.cursor/rules/playwright-conventions.mdc)):
...
1. The **setup** project runs [`tests/auth.setup.ts`](tests/auth.setup.ts), logs in with `DIDAXIS_EMAIL` / `DIDAXIS_PASSWORD`, and saves session cookies to `playwright/.auth/user.json`.
...
reports/         Eval reliability reports (generated by CI / npm run eval-report)
.cursor/
...
.github/workflows/
  e2e.yml        Runs tagged slices on push/PR; full suite on manual dispatch
...
1. **Open this repo in Cursor** — rules under [`.cursor/rules/`](.cursor/rules/) load automatically ([`constitution.mdc`](.cursor/rules/constitution.mdc), [`qa-orchestration.mdc`](.cursor/rules/qa-orchestration.mdc), [`playwright-conventions.mdc`](.cursor/rules/playwright-conventions.mdc)).
...
3. **Configure MCP** in Cursor settings (Settings → MCP) so the agent can reach external systems:
   ...
   - **Playwright / browser** — live browser / a11y tree for exploration and self-heal

4. **Typical local flow** (orchestrated by [`qa-orchestration.mdc`](.cursor/rules/qa-orchestration.mdc)):
...
Hooks ([`.cursor/hooks.json`](.cursor/hooks.json)) run on file edits and enforce project guardrails — e.g. no CSS/XPath locators, no weakened assertions, test-writer stays under `tests/`.

Skills and agents live in [`.cursor/skills/`](.cursor/skills/) and [`.cursor/agents/`](.cursor/agents/); the coordinator reads them automatically when relevant.

## CI & GitHub secrets

Configure secrets under **Repository → Settings → Secrets and variables → Actions**. Cursor Cloud/API keys are separate from repo secrets — store `CURSOR_API_KEY` in both GitHub Actions and your local `.env` if you run the agent locally.

### E2E Tests ([`.github/workflows/e2e.yml`](.github/workflows/e2e.yml))

Runs on push, pull request, and manual dispatch (`workflow_dispatch`):

| Trigger | What runs |
|---------|-----------|
| Pull request | `@smoke` (`npm run test:smoke`) |
| Push | `@sanity` (`npm run test:sanity`) |
| Manual dispatch | Full suite (`npx playwright test`) |
...
### Test Generation ([`.github/workflows/test-generation.yml`](.github/workflows/test-generation.yml))
...
The workflow uploads `reports/eval-report.md` as a reliability artifact after each run.

## Useful scripts

```bash
# Tagged slices (see playwright-conventions.mdc)
npm run test:smoke
npm run test:sanity
npm run test:regression
npm run test:api
...
# Reliability report (CI runs this after test-generation)
npm run eval-report
...
- **Role-based locators only** (`getByRole`, `getByLabel`, `getByText`) — see [`.cursor/rules/playwright-conventions.mdc`](.cursor/rules/playwright-conventions.mdc)
- **Test data** via `test-data/factories/` + automatic cleanup via `trackProgram` in [`fixtures/cleanup.fixture.ts`](fixtures/cleanup.fixture.ts)
