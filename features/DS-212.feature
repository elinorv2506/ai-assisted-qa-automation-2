Feature: DS-K1 — Add user in Settings
  DS-212 — Admin can add a new user from the Settings page Users section
  ...
  @TC-002 @AC-AddUserForm
  Scenario: Add User modal displays required fields
    Given I am logged in as admin
  ...
  # - Ticket description is a single line with no formal Gherkin AC; scenarios inferred from Settings UI (Users table, Add User modal).
  # - No DELETE /users endpoint observed; cleanup uses PATCH is_active=false after tests.
  # - Duplicate-email error copy not specified in ticket; test asserts generic validation/error visibility.
  # - Non-admin access to Users section not specified; ADMIN-only gating inferred from app role wrapper.
